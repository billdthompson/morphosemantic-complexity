...
\item \textbf{Morphological Sum} --- The sum of the total values for each category, e.g., Feminine, Masculine, Past, ...

\item \textbf{GZIP-R} --- A measure of morphological complexity similar to $D_{structure}$ \cite{koplenig2017statistical}: [1-size of gzipped-plain-text]/[size of gzipped-word-substituted-text] where word-substituted text is created by replacing each word with a random number of characters drawn from the frequency distribution of characters in the language. This results in disrupting compression gains that are based on reusing codes for stems in morphologically derived words.
\end{itemize}
...
Figure \ref{correlations} shows simple Pearson correlations between the variables. Several of these are worth highlighting: a) the number of categories is a rather bad predictor of all measures of morphological complexity because almost all the languages in the sample have many of the same morphological categories, differing on the number of values per category; b) The proportion of word forms that are equal to their lemmas (both as raw wordforms and proportion of unique wordforms) correlates to a surprising extent with previously published WALS-based measure ($C_{WALS}$) and entropy-based measures ($D_{structure}$), as well as our own entropy-based measure (GZIP-R); c) both our semantic distance and semantic variance measures are strongly correlated with $C_{WALS}$, $D_{structure}$ and GZIP-R. Table \ref{tab:tabel1} shows a subset of these measures for the ten most and least complex langauges, as judged by our \emph{Semantic Distance (All Tokens)} measure.
...
As an initial test of the kind of small differences in complexity our semantic-distance measures is able to detect, we examined the closely-related languages Bokm\aaål and Nynorsk (we also studied Serbian/Croatian, and found similar subtelties). Bokm\aaål (lit. Book tongue) is a form of Norwegian primarily used in writing, and one that, of the two varieties, has been strongly influenced by Danish. Nynorsk is a minority form used by ~12.5\% of Norwegians and one that has resisted Danish influence to a greater extent. The treebanks for the two varieties are nearly the same size and show almost identical categories and values. Bokm\aaål has two more values (reflexives and a passive voice) and so on this measure may be viewed as being slightly more complex, a conclusion also supported by Koplenig's entropy-based measure of structural complexity of bible translations ($D_{structure}$ Bokm\aaål = .24; $D_{structure}$ Nynorsk=.22), as well as our own entropy-based estimate. In contrast, the semantic-distance measure we compute here gives more sensible estimates: Bokm\aaål is simpler -- it has lower semantic variance (i.e., having more semantically consistent morphological paradgims): Bokm\aaål = 10.65, Nynorsk=14.24. Consistent with Bokm\aaål being strongly influenced by Danish, its semantic variance is very close to Danish (10.81). Note that some variables, such as the pasive voice and reflexives, are present in the Nynorsk language but not annotated in the universal depencies dataset, and therefore cannot be accounted for here.
